/*
 * PerSender fairness policy VS. Reactive Attacks
 */
...
std::ofstream windowFile ("persender_reactive_window.data", std::ios::out);
std::ofstream lossRateFile ("persender_reactive_LR.data", std::ios::out);
#define AS_count 3
double AS_window[AS_count];
double attacker_sent[AS_count];
double attacker_dropped[AS_count];
...
void clearArray(){
  ...
  for (uint32_t j = 0; j < AS_count; ++j){
    AS_window[j] = 0;
    attacker_sent[j] = 0;
    attacker_dropped[j] = 0;
  }
  total_capacity = 0;
}
...
PktArrival (Ptr<const Packet> p)
{
  ...
  if (pktCopy -> PeekPacketTag(tag)) {
    ...
    if (enableEarlyDrop > 0) {
      ...
      if (receiveWin[index] > congWin[index]) { // best-effort
  ...
  if (Simulator::Now().GetSeconds() > detectPeriod) {
	...
	lossRateArray[j] = beta * lossRateArray[j];
      ...
      if (receiveWin[j] >= dropArray[j]) total_capacity += (receiveWin[j] - dropArray[j]);
    ...
    for (uint32_t j = 0; j < nLeaf + nAttacker; ++j){
      if (j == 0) {
	congWin[j] = total_capacity / (1 + AS_count);
      }
      else {
	congWin[j] = (total_capacity - congWin[0]) / nAttacker;
	AS_window[j % 3] += congWin[j];
	attacker_sent[j%3] += receiveWin[j];
	attacker_dropped[j%3] += dropArray[j];
      }
    }

    if (windowFile.is_open())
    {
      std::cout << "Client NO." << 0 << "; congestion window: " << congWin[0] << std::endl;
      windowFile << congWin[0] << " ";
      for (uint32_t j = 0; j < AS_count; ++j){
	windowFile << AS_window[j] << " ";
	std::cout << "Client NO." << j+1 << "; congestion window: " << AS_window[j] << std::endl;
      }
      windowFile << std::endl;
    }
    ...
    if (lossRateFile.is_open())
    {
      lossRateFile << lossRateArray[0] << " ";
      for (uint32_t j = 0; j < AS_count; ++j){
	lossRateFile << attacker_dropped[j] / attacker_sent[j] << " ";
      }
      lossRateFile << std::endl;
    }
...
main (int argc, char *argv[])
{
  ...
  std::string attackFlowType = "ns3::TcpSocketFactory";
  std::string mtu = "1599";
  ...
  //for (; start < stopTime; start += period){
    ...
    Simulator::Schedule(Seconds (0), &attackFlow, d, nAttacker, stopTime);
  //}
  ...
  {
    outputFile << "==============================================================="
	       << "\nPer sender fariness with reactive rate: "
	       ...
	       << "\nBeta: " << beta
	       << "\ncTable size: " << realtimePacketFeedback
