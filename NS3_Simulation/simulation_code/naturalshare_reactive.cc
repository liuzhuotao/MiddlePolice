/*
 * NaturalShare Policy VS. Reactive Attacks
 */
...
std::ofstream windowFile ("natural_reactive_window.data", std::ios::out);
std::ofstream lossRateFile ("natural_reactive_LR.data", std::ios::out);
std::ofstream lossRateFile1 ("natural_reactive_LR1.data", std::ios::out);
#define AS_count 3
double attacker_windows[AS_count];
...
PktArrival (Ptr<const Packet> p)
{
  ...
  if (Simulator::Now().GetSeconds() > detectPeriod) {
	...
	lossRateArray[j] = beta * lossRateArray[j];
      ...
      if (j > 0) {
	attacker_windows[j % 3] += congWin[j];
    ...
    if (windowFile.is_open()) {
      ...
      for (uint32_t j = 0; j < AS_count; ++j){
	windowFile << attacker_windows[j] << " ";
	std::cout << "Client NO." << j+1 << "; congestion window: " << attacker_windows[j] << std::endl;
      }
      windowFile << "\n";
    }
...
main (int argc, char *argv[])
{
  ...
  {
    outputFile << "==============================================================="
	       << "\nNaturalShare with reactive rate: "
