/*
 * Per-sender Fairness Policy VS. Shrew Attack
 */
...
double period = 4.0;
double attack_period = 4.0;
double duration = attack_period / 2;
uint16_t port = 5001;
...
std::ofstream windowFile ("persender_flat_window.data", std::ios::out);
std::ofstream lossRateFile ("persender_flat_LR.data", std::ios::out);
...
PktArrival (Ptr<const Packet> p)
{
  ...
  if (Simulator::Now().GetSeconds() > detectPeriod) {
    ...
    // rate allocation
    for (uint32_t j = 0; j < nLeaf + nAttacker; ++j){
      congWin[j] = total_capacity / (nLeaf + nAttacker);
    }

    for (uint32_t j = 0; j < nLeaf + nAttacker; ++j){
      if (windowFile.is_open())
      {
	if ((receiveWin[j] - dropArray[j]) > congWin[j]) 
	  windowFile << (receiveWin[j] - dropArray[j]) << " ";
	else 
...
main (int argc, char *argv[])
{
  ...
  for (; start < stopTime; start += period){
    ...
    Simulator::Schedule(Seconds (start), &attackFlow, d, nAttacker);
  }
  ...
  {
    outputFile << "==============================================================="
	       << "\nPer sender fariness with shrew rate: "
	       ...
	       << "\nAttack period: " << attack_period << ", attack duration: " << duration 
